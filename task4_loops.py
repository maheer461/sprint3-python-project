...
# a) Print games that sold more than 25 million copies globally
print("Games with global sales > 25 million:")
for game in video_game_sales:
    if game[GLOBAL_SALES] > 25:
        print(f"{game[NAME]}: {game[GLOBAL_SALES]} million")
...
# c) Calculate total NA sales and total Japan sales
total_na_sales = 0
total_jp_sales = 0
...
print(f"\nTotal North America sales: ${total_na_sales:.2f} million")
...
# d) Create list of Nintendo-published games
nintendo_games = []
for game in video_game_sales:
    if game[PUBLISHER] == 'Nintendo':
        nintendo_games.append(game[NAME])

print(f"\nNintendo games ({len(nintendo_games)} titles):")
