...
# a) Function to calculate total of NA, EU, and JP sales
...
print(f"Total regional sales for {first_game[NAME]}: ${total_regional_sales:.2f} million")
print()

# b) Function to filter games by genre (default: 'Platform')
...
print(f"Platform games (default): {len(platform_games)} games")
...
print(f"Sports games: {len(sports_games)} games")
for game in sports_games:
    print(f"  - {game[NAME]} ({game[YEAR]})")
print()

# c) Function to return formatted summary string
def get_summary(game):
    return f"{game[NAME]} ({game[YEAR]}) - {game[GENRE]} - ${game[GLOBAL_SALES]}M"

# Use the function to print summary of every game
